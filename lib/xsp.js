...
(function() {
  var app, async, cfgmore, cfgupdate, client, controller, controllers, dictionaries, express, fce, fs, http, https, inits, loadModule, loaded, loadedQueue, locals, name, pathsTo, report, route, routed, socketClient, socketController, tasks, xsp, yaml, _ref,
  ...
  global.trace = function(msg) {
    return console.log(Array.prototype.join.call(arguments, ', '));
  };
  ...
  if (fs.existsSync("" + __dirname + "/../../../app/config/server." + xsp.env + ".yml")) {
    ...
    cfgupdate = function(tar, src) {
      ...
      for (name in src) {
        value = src[name];
        if (tar[name]) {
          if (typeof tar[name] === 'object' && src[name] === 'object') {
  ...
  loadModule = function(module, loadedpath) {
    var deps, index, path, _results;
    ...
    _results = [];
    for (path in loadedQueue) {
      module = loadedQueue[path];
      ...
      if (index !== -1) {
        ...
        if (deps.length === 0) {
          ...
          _results.push(loadModule(module, path));
        } else {
  ...
  xsp.autoload = function(dir, scope) {
    var ex, l, later, _i, _len, _ref, _ref1, _results;

    if (fs.existsSync("" + dir)) {
      if (fs.lstatSync(dir).isDirectory()) {
        later = [];
        fs.readdirSync(dir).forEach(function(file) {
          var dep, deps, ex, index, module, path, stats, _i, _j, _len, _ref;

          path = require('path').normalize("" + dir + "/" + file);
          stats = fs.lstatSync(path);
          if (file.substr(0, 1) !== '.') {
            if (stats.isDirectory()) {
              return later.push(path);
            } else {
              try {
                if (scope) {
                  module = require(path);
                  if ((_ref = module.DEPENDENCIES) == null) {
                    module.DEPENDENCIES = [];
                  }
                  if (typeof module.DEPENDENCIES === 'string') {
                  ...
                  if (deps.length > 0) {
                    for (index = _j = deps.length - 1; _j >= 0; index = _j += -1) {
                      dep = deps[index];
                      if (loaded[dep]) {
                        deps.splice(index, 1);
                      }
                  ...
                  if (deps.length === 0) {
                    ...
                    return loadedQueue[path] = module;
                  }
                } else {
                  return require(path);
                }
        ...
        for (_i = 0, _len = later.length; _i < _len; _i++) {
          l = later[_i];
          _results.push(xsp.autoload(l, scope));
        }
        return _results;
      } else {
        try {
      ...
      if (fs.existsSync("" + dir + ".js")) {
        try {
          if (scope) {
            return (_ref1 = require(dir)) != null ? _ref1.call(xsp) : void 0;
          } else {
            return require(dir);
          }
        } catch (_error) {
  ...
  xsp.get = function() {
    var arg, controller, index, path, re, route, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
  ...
  xsp.post = function() {
    var arg, controller, index, path, re, route, _i, _j, _k, _len, _len1, _len2, _ref;
  ...
  xsp.all = function() {
    var arg, controller, index, path, re, route, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1;
  ...
  xsp.path = function(controller, lang) {
    var anchor, args, index, last, path;
  ...
  routed = function(req, res, next) {
    var l, n, render, _ref, _ref1, _ref2, _ref3;

    req.language = (_ref = (_ref1 = (_ref2 = req.params) != null ? _ref2.lang : void 0) != null ? _ref1 : (_ref3 = req.cookies) != null ? _ref3.language : void 0) != null ? _ref : xsp.cfg.default_locale;
